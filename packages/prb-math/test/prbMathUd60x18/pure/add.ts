...
export default function shouldBehaveLikeAdd(): void {
  context("when the sum overflows", function () {
    const testSets = [
      [toBn("1e-18"), toBn(MAX_UD60x18)],
      [toBn(MAX_UD60x18).div(2), toBn(MAX_UD60x18).div(2).add(2)],
      [toBn(MAX_UD60x18).div(2).add(2), toBn(MAX_UD60x18).div(2)],
      [toBn(MAX_UD60x18), toBn("1e-18")],
    ];
  ...
  context("when the sum does not overflow", function () {
    const testSets = [
      ...
      [toBn(MAX_UD60x18).sub(1), toBn("1e-18")],
    ];
