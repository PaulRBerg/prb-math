...
export default function shouldBehaveLikeCeil(): void {
  ...
  context("when x is not zero", function () {
    context("when x > max whole ud60x18", function () {
      const testSets = [[toBn(MAX_WHOLE_UD60x18).add(1)], [toBn(MAX_UD60x18)]];
    ...
    context("when x <= max whole ud60x18", function () {
      ...
      forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
        ...
        expect(expected).to.equal(await this.contracts.prbMathUd60x18.doCeil(toBn(x)));
        expect(expected).to.equal(await this.contracts.prbMathUd60x18Typed.doCeil(toBn(x)));
      });
    });
  });
}
