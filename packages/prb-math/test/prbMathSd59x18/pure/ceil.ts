  ...
  context("when x is not zero", function () {
    context("when x is negative", function () {
      ...
      forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
        const expected: BigNumber = toBn(ceil(x));
        expect(expected).to.equal(await this.contracts.prbMathSd59x18.doCeil(toBn(x)));
        expect(expected).to.equal(await this.contracts.prbMathSd59x18Typed.doCeil(toBn(x)));
      });
    });

    context("when x is positive", function () {
      context("when x > max whole sd59x18", function () {
        const testSets = [[toBn(MAX_WHOLE_SD59x18).add(1)], [toBn(MAX_SD59x18)]];
