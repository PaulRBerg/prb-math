...
export default function shouldBehaveLikeExp2(): void {
  ...
  context("when x is negative", function () {
    context("when x is less than -59.794705707972522261", function () {
      const testSets = [toBn("-59.794705707972522262"), toBn(MIN_WHOLE_SD59x18), toBn(MIN_SD59x18)];
    ...
    context("when x is greater than or equal to -59.794705707972522261", function () {
      ...
      forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
        const expected: BigNumber = toBn(exp2(x));
        expect(expected).to.be.near(await this.contracts.prbMathSd59x18.doExp2(toBn(x)));
        expect(expected).to.be.near(await this.contracts.prbMathSd59x18Typed.doExp2(toBn(x)));
      });
    });
  });
  ...
  context("when x is positive", function () {
    context("when x is greater than or equal to 192", function () {
      const testSets = [toBn("192"), toBn(MAX_WHOLE_SD59x18), toBn(MAX_SD59x18)];
