...
export default function shouldBehaveLikeExp(): void {
  ...
  context("when x is negative", function () {
    context("when x is less than -41.446531673892822322", function () {
      const testSets = [toBn("-41.446531673892822323"), toBn(MIN_WHOLE_SD59x18), toBn(MIN_SD59x18)];
    ...
    context("when x is greater than or equal to -41.446531673892822322", function () {
      ...
      forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
        ...
        expect(expected).to.be.near(await this.contracts.prbMathSd59x18.doExp(toBn(x)));
        expect(expected).to.be.near(await this.contracts.prbMathSd59x18Typed.doExp(toBn(x)));
      });
    });
  });
  ...
  context("when x is positive", function () {
    context("when x is greater than or equal to 133.084258667509499440", function () {
      const testSets = [toBn("133.084258667509499441"), toBn(MAX_WHOLE_SD59x18), toBn(MAX_SD59x18)];
