  ...
  context("when x is not zero", function () {
    context("when x is negative", function () {
      context("when x < min whole sd59x18", function () {
        const testSets = [[toBn(MIN_SD59x18)], [toBn(MIN_WHOLE_SD59x18).sub(1)]];
      ...
      context("when x >= min whole sd59x18", function () {
        ...
        forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
          ...
          expect(expected).to.equal(await this.contracts.prbMathSd59x18.doFloor(toBn(x)));
          expect(expected).to.equal(await this.contracts.prbMathSd59x18Typed.doFloor(toBn(x)));
        });
      });
    });
