...
export function shouldBehaveLikeLn(): void {
  context("when x is zero", function () {
    it("throws an error", function () {
      ...
      expect(() => ln(x)).to.throw("Cannot calculate the natural logarithm of zero");
    });
  });
  ...
  context("when x is not zero", function () {
    context("when x is negative", function () {
      it("throws an error", function () {
        ...
        expect(() => ln(x)).to.throw("Cannot calculate the natural logarithm of a negative number");
      });
    });
    ...
    context("when x is positive", function () {
      ...
      forEach(testSets).it("takes %f and returns the correct value", function (x: string) {
        ...
        const result: number = Number(ln(toBn(x)));
