...
export default function shouldBehaveLikeDiv(): void {
  ...
  context("when the denominator is negative", function () {
    context("when the scaled numerator overflows", function () {
      it("reverts", async function () {
        const x: BigNumber = MIN_58x18.div(UNIT).sub(1);
        const y: BigNumber = fp(-0.000000000000000001);
        await expect(this.contracts.prbMath.doDiv(x, y)).to.be.reverted;
      });
    });
    ...
    context("when the scaled numerator does not overflow", function () {
      context("when the numerator is zero", function () {
        const testSets = [fp(-0.000000000000000001), fp(-1), PI.mul(-1), bn(-1e36)];
      ...
      context("when the numerator is negative", function () {
        const testSets = [
      ...
      context("when the numerator is positive", function () {
        const testSets = [
          [fp(0.000000000000000001), MIN_58x18, ZERO],
          [fp(0.000000000000000001), fp(-1).sub(1), ZERO],
          [fp(0.000000000000000001), fp(-1), fp(-0.000000000000000001)],
  ...
  context("when the denominator is positive", function () {
    context("when the scaled numerator overflows", function () {
      it("reverts", async function () {
        const x: BigNumber = MAX_58x18.div(UNIT).add(1);
        const y: BigNumber = fp(0.000000000000000001);
        await expect(this.contracts.prbMath.doDiv(x, y)).to.be.reverted;
      });
    });
