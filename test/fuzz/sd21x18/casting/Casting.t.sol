...
import { uMIN_SD1x18, uMAX_SD1x18 } from "src/sd1x18/Constants.sol";
import { sd21x18, wrap, unwrap } from "src/sd21x18/Casting.sol";
import { SD21x18 } from "src/sd21x18/ValueType.sol";
import { SD59x18 } from "src/sd59x18/ValueType.sol";
import { MAX_SD21x18, MIN_SD21x18 } from "src/sd21x18/Constants.sol";
import {
    PRBMath_SD21x18_IntoSD1x18_Overflow,
    PRBMath_SD21x18_IntoSD1x18_Underflow,
...
contract Casting_Fuzz_Test is Base_Test {
    function testFuzz_RevertWhen_OverflowSD1x18(SD21x18 x) external {
        x = _bound(x, int128(uMAX_SD1x18) + 1, MAX_SD21x18);
        vm.expectRevert(abi.encodeWithSelector(PRBMath_SD21x18_IntoSD1x18_Overflow.selector, x));
        x.intoSD1x18();
    }

    function testFuzz_RevertWhen_UnderflowSD1x18(SD21x18 x) external {
        x = _bound(x, MIN_SD21x18, int128(uMIN_SD1x18) - 1);
        vm.expectRevert(abi.encodeWithSelector(PRBMath_SD21x18_IntoSD1x18_Underflow.selector, x));
        x.intoSD1x18();
    }

    function testFuzz_IntoSD1x18(SD21x18 x) external {
        x = _bound(x, int128(uMIN_SD1x18), uMAX_SD1x18);
        SD1x18 actual = x.intoSD1x18();
        SD1x18 expected = SD1x18.wrap(int64(x.unwrap()));
        assertEq(actual, expected, "SD59x18 intoSD1x18");
    }

    function testFuzz_IntoSD59x18(SD21x18 x) external {
        SD59x18 actual = x.intoSD59x18();
        SD59x18 expected = SD59x18.wrap(int256(x.unwrap()));
        assertEq(actual, expected, "SD21x18 intoSD59x18");
    }

    function testFuzz_RevertWhen_UnderflowUD2x18(SD21x18 x) external {
        x = _bound(x, MIN_SD21x18, -1);
        vm.expectRevert(abi.encodeWithSelector(PRBMath_SD21x18_ToUD2x18_Underflow.selector, x));
        x.intoUD2x18();
    }

    function testFuzz_IntoUD2x18(SD21x18 x) external {
        x = _bound(x, 0, MAX_SD21x18);
        UD2x18 actual = x.intoUD2x18();
        UD2x18 expected = UD2x18.wrap(uint64(uint128(x.unwrap())));
        assertEq(actual, expected, "SD21x18 intoUD2x18");
    }
    ...
    function testFuzz_IntoUD60x18(SD21x18 x) external {
        x = _bound(x, 0, MAX_SD21x18);
    ...
    function testFuzz_IntoUint256(SD21x18 x) external {
        x = _bound(x, 0, MAX_SD21x18);
    ...
    function testFuzz_IntoUint128(SD21x18 x) external {
        x = _bound(x, 0, MAX_SD21x18);
    ...
    function testFuzz_IntoUint40(SD21x18 x) external {
