...
import {
    ...
    PRBMath_SD1x18_ToUD21x18_Underflow,
...
contract Casting_Fuzz_Test is Base_Test {
    function testFuzz_IntoSD21x18(SD1x18 x) external pure {
    ...
    function testFuzz_RevertWhen_UnderflowUD21x18(SD1x18 x) external {
        x = _bound(x, MIN_SD1x18, -1);
        vm.expectRevert(abi.encodeWithSelector(PRBMath_SD1x18_ToUD21x18_Underflow.selector, x));
        x.intoUD21x18();
    }

    function testFuzz_IntoUD21x18(SD1x18 x) external pure {
        x = _bound(x, 0, MAX_SD1x18);
        UD21x18 actual = x.intoUD21x18();
        UD21x18 expected = UD21x18.wrap(uint64(x.unwrap()));
        assertEq(actual, expected, "SD1x18 intoUD21x18");
    }
