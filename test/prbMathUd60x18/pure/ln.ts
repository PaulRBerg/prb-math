...
export default function shouldBehaveLikeLn(): void {
  ...
  context("when x is greater than or equal to 1", function () {
    ...
    forEach(testSets).it("takes %e and returns the correct value", async function (x: string) {
      ...
      expect(expected).to.be.near(await this.contracts.prbMathUd60x18.doLn(toBn(x)));
      expect(expected).to.be.near(await this.contracts.prbMathUd60x18Typed.doLn(toBn(x)));
    });
  });
}
