...
export default function shouldBehaveLikeFromUint(): void {
  context("when x is greater than max ud60x18 divided by scale", function () {
    ...
    forEach(testSets).it("takes %e and reverts", async function (x: BigNumber) {
      await expect(this.contracts.prbMathUd60x18.doFromUint(x)).to.be.revertedWith(PRBMathErrors.FromUintOverflow);
      await expect(this.contracts.prbMathUd60x18Typed.doFromUint(x)).to.be.revertedWith(PRBMathErrors.FromUintOverflow);
    });
  });
